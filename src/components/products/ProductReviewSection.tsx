...
const ProductReviewSection: React.FC<ProductReviewSectionProps> = ({
  ...
  const uploadImages = async (): Promise<string[]> => {
    ...
    for (const file of selectedImages) {
      const safeName = `${currentUser?.id || "anon"}_${date}_${Date.now()}_${
        file.name
      }`;
  ...
  return (
    <section className="w-full  mx-auto space-y-6 px-1 sm:px-2">
      <div className="bg-zinc-950/60 backdrop-blur-md rounded-none p-3 border border-zinc-800 shadow-inner">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h3 className="text-md lg:text-xl font-semibold text-white tracking-tight">
              Customer Reviews
            </h3>
        ...
        {showWriteReview && (
          <div className="mt-2 bg-zinc-900/70 border border-zinc-800 rounded-xl p-2 shadow-md">
            <div className=" items-start justify-between">
              <h4 className="text-sm lg:text-lg font-medium text-white">
                Write your review
              </h4>
              <div className="text-xs lg:text-lg font-medium text-zinc-400">
                Optional: add up to 3 images
              </div>
            </div>

            <div className="mt-2 grid grid-cols-1 gap-4">
              <div>
                <Label className="text-sm lg:text-md text-zinc-300">
                  Your rating
                </Label>
              ...
              <div>
                <Label className="text-xs lg:text-md text-zinc-300">
                  Images (optional)
                </Label>
