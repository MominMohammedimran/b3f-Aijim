...
import { ShoppingCart, User, Search, Package, Home, PackageSearch, Tag } from 'lucide-react';
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const loadCartCount = async () => {
      if (currentUser) {
        const { data } = await supabase.from('carts').select('*').eq('user_id', currentUser.id);
  ...
  const handleSignOut = async () => {
    try {
      if (currentUser) await supabase.from('carts').delete().eq('user_id', currentUser.id);
      clearCart();
        ...
        <Marquee gradient={false} speed={40} pauseOnHover className="w-full">
          {Array.from({ length: 10 }).map((_, i) => (
            ...
            >
              ...
              "Every T-shirt tells a story — built from hustle. #<span className="font-bold ">AIJIM</span>”
