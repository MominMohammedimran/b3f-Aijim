...
const ProductVideoSection: React.FC = () => {
  ...
  // ⭐ Function to Open Fullscreen: Unmutes if currently muted.
  const openFullscreen = (id: number) => {
    const video = videoRefs.current[id];
    if (video && mutedStates[id]) {
      // Unmute the video when going fullscreen
      toggleMute(id);
    }
    setIsFullscreenOpen(true);
  };

  // ⭐ Function to Close Fullscreen: Re-mutes if unmuted while in fullscreen.
  const closeFullscreen = useCallback(() => {
    if (!mutedStates[activeVideo]) {
      // Mute video again when closing fullscreen, unless the user had already unmuted it
      ...
      {isFullscreenOpen && activeVideoObj && (
        ...
        >
          ...
          <div className="relative w-full h-full max-w-[90vw] max-h-[90vh] flex items-center justify-center">
            {/* The video element for the modal */}
            <video
              ...
              onLoadedMetadata={(e) => {
                // Ensures the modal's video matches the current mute state
